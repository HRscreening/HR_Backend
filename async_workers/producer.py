...
class ARQProducer:
    ...
    def __init__(self,redis:ArqRedis):
        ...
        self.resume_parsing_config = JobConfig(
            ...
            worker_func="parse_resume_worker",
        )
        
        
        self.resume_scoring_text_config = JobConfig(
            queue_name= self.default_queue ,
            worker_func = "score_resumes_text_batch",
        )
        self.resume_scoring_url_config = JobConfig(
            queue_name= self.default_queue ,
            worker_func = "score_resumes_url_batch",
        )
        
        
        self.candidate_extraction_config = JobConfig(
                queue_name = self.default_queue ,
                worker_func = "extract_candidate_worker",
        )

        self.logger = get_logger("JOB_PRODUCER")


    # =========================
    # Resume Parsing
    # =========================
    async def enqueue_resumes_parsing(
        self,
